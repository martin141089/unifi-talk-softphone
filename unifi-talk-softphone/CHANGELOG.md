...
### Diagnose
- **Weiterhin kein Ton (beide Richtungen) trotz 0.5.1-Fix** - Live-Logs zeigen,
  dass die Console in ihrer SDP bereits `talk_sip_host` als RTP-Ziel meldet
  (die IP-Umbiegung aus 0.5.1 greift also gar nicht erst, war für dieses
  Setup nicht die Ursache). ICE zwischen Browser und Add-on läuft dabei
  erfolgreich über eine direkte LAN-Kandidatenpaarung durch (TURN wäre für
  diesen Fall nicht nötig gewesen). Das eigentliche Problem liegt damit
  entweder in der SIP/RTP-Strecke zwischen Add-on und Console (z. B. Firewall
  blockiert die dynamischen RTP-Ports, oder die Console routet die tatsächlichen
  Medien-Pakete bei extern gebridgten Anrufen über einen anderen internen Pfad
  als die gemeldete SDP-IP) oder in der Opus↔G.711-Bridge selbst.
- `RtpSession` (`sip_client.py`) zählt jetzt gesendete/empfangene RTP-Pakete
  und loggt das erste Paket je Richtung sowie alle 250 weiteren, plus eine
  Zusammenfassung beim Auflegen. Damit lässt sich beim nächsten Testanruf aus
  den Logs ablesen, ob überhaupt RTP in eine/beide Richtungen ankommt, statt
  nur zu wissen "kein Ton".
